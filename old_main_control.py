...
# ===== BALANCE PARAMETERS =====
# Tuned for stable, non-wandering balance
BALANCE_KP = 7.5   # Reduced from 8.6 - less aggressive, more stable
BALANCE_KI = 0.15  # Reduced from 0.22 - prevent integral windup/wandering
BALANCE_KD = 0.06  # Increased from 0.02 - more damping to prevent oscillation

# ===== STEERING PARAMETERS =====
# Very gentle steering - focus on balance first
STEER_KP = 10.0    # Lower than balance Kp - steering is secondary
STEER_KI = 0.05    # Minimal integral for steering
STEER_KD = 0.02    # Some damping for smooth transitions

THETA_MAX_DEG = 0.5  # Half degree max lean for steering
...
def main():
    ...
    pid_x = PID(
        ...
        integ_min=-0.15, integ_max=0.15,  # Tighter integral limits to prevent wandering
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                if BALANCE_MODE:
                    # Tune balance PIDs in balance mode
                    ...
                    # Tune steering PIDs in steering mode
                    if dpad_vert > prev_dpad_vert:
                        if cur_tune == 0:
                            pid_x_steer.kp += step
                            pid_y_steer.kp += step
                ...
                if BALANCE_MODE:
                    print(
                        f"[BALANCE] Time: {t_now:.3f}s{abort_str} | "
                        f"θx: {np.rad2deg(theta_x):+6.2f}°, θy: {np.rad2deg(theta_y):+6.2f}° | "
                        f"BAL PID: Kp={pid_x.kp:.3f}, Ki={pid_x.ki:.4f}, Kd={pid_x.kd:.4f} | "
                        f"Tx: {Tx:+6.3f}, Ty: {Ty:+6.3f} | "
                        f"u=[{u1:+5.3f},{u2:+5.3f},{u3:+5.3f}]"
                    )
                    print(f"    Tuning BALANCE {tune_names[cur_tune]} (D-pad ↑/↓ adjust, ←/→ switch)")
                else:
                    print(
                        f"[STEER] Time: {t_now:.3f}s{abort_str} | "
                    ...
                    print(f"    BAL: Kp={pid_x.kp:.2f}, Ki={pid_x.ki:.3f}, Kd={pid_x.kd:.3f} | "
                          f"STEER: Kp={pid_x_steer.kp:.2f}, Ki={pid_x_steer.ki:.3f}, Kd={pid_x_steer.kd:.3f}")
                    print(f"    Tuning STEERING {tune_names[cur_tune]} (D-pad ↑/↓ adjust, ←/→ switch)")
