...
BALANCE_INTEG_MAX = 0.15

# Drift correction (outer loop - position stabilization)
# IMU drift is typically 0.5-1.5° over 20-30 seconds
# These VERY gentle gains only correct a FRACTION of drift to reduce runaway
# They don't fully correct - just slow down the drift rate
DRIFT_KP = 0.015         # Very gentle position feedback (only corrects ~20% of drift)
DRIFT_KD = 0.003         # Very gentle velocity damping
DRIFT_MAX_ANGLE = 0.01   # Max correction angle: ~0.57° (much less than IMU drift)
            ...
            try:
                ...
                if BALANCE_MODE:
                    if DRIFT_CORRECTION_ENABLED:
                        # Gentle drift correction: tilt slightly to counteract position drift
                        drift_x = DRIFT_KP * (-phi_x) + DRIFT_KD * (-dphi_x)
                        drift_y = DRIFT_KP * (-phi_y) + DRIFT_KD * (-dphi_y)
